...
func TestAllEndpointStates_Unmarshal(t *testing.T) {
	tests := []struct {
		name string
		e    AllEndpointStates
		data string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := json.Unmarshal([]byte(tt.data), &tt.e); err != nil || tt.e == nil {
				spew.Dump(tt.e)
...
func TestCassResponse_Unmarshal(t *testing.T) {
	tests := []struct {
		name string
		e    CassResponse
		data string
		...
		{
			name: "unmarshalls prettified SimpleStates and encoded AllEndpointStates",
			data: `{
						"SimpleStates": {
							"/10.244.0.6": "UP",
							"/10.244.0.7": "UP",
