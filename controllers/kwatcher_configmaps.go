...
func (r *CassandraClusterReconciler) reconcileKwatcherKeyspaceConfigMap(ctx context.Context, cc *v1alpha1.CassandraCluster) error {
	...
	if err != nil && kerrors.IsNotFound(err) {
		r.Log.Info("Creating kwatcher keyspaces ConfigMap")
		if err = r.Create(ctx, desiredCM); err != nil {
			return errors.Wrap(err, "Unable to create kwatcher keyspaces ConfigMap")
		}
	} else if err != nil {
		return errors.Wrap(err, "Could not Get kwatcher keyspaces ConfigMap")
	} else if !compare.EqualConfigMap(actualCM, desiredCM) {
		r.Log.Info("Updating kwatcher keyspaces ConfigMap")
		...
		if err = r.Update(ctx, actualCM); err != nil {
			return errors.Wrap(err, "Could not Update kwatcher keyspaces ConfigMap")
		}
	} else {
